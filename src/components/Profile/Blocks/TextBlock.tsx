...
interface TextBlockProps {
  toggleTextBlock: boolean;
  setToggleTextBlock: Dispatch<SetStateAction<boolean>>;
  onSelect: () => void;
}
export const TextBlock = forwardRef(
  (
    { toggleTextBlock, setToggleTextBlock, onSelect }: TextBlockProps,
    ref: ForwardedRef<any>
  ) => {
    const handleClick = (event: React.MouseEvent<HTMLDivElement>) => {
      event.preventDefault();
      setToggleTextBlock(true);
      onSelect();
    };
    ...
    return (
      <div onClick={handleClick} className="flex">
        <ul className="space-y-4 w-64 pt-2 ">
          <button
            onClick={() => setToggleTextBlock(true)}
            className={`w-full ${
              toggleTextBlock ? "text-blue-600 bg-gray-100" : "text-gray-700"
